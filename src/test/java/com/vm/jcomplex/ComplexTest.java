 ...
 * contributor license agreements. This file is a modified version of ComplexTest.java
...
public class ComplexTest {
    ...
    public void testJava() {// TODO more debug
        ...
        // final long exponentOffset = 1023l;
        // final double epsilon = Double.longBitsToDouble((exponentOffset - 53l) << 52);
        // Assert.assertEquals("exp of -infinity should be 0.0", 0.0, Math.exp(Double.NEGATIVE_INFINITY), epsilon);
    ...
    public void testSqrtPolar() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            for (int j =0; j < 11; j++) {
                theta += pi /12;
                Complex z = Complex.fromPolar(r, theta);
                Complex sqrtz = Complex.fromPolar(Math.sqrt(r), theta / 2);
    ...
    public void testSerial() {
        ...
        Number number = new Complex(3.0, 4.0);
        Complex recoveredNumber = (Complex)TestUtils.serializeAndRecover(number);
        Assert.assertEquals(number, recoveredNumber);
        Assert.assertEquals(3.0, recoveredNumber.getReal(), 0);
        Assert.assertEquals(4.0, recoveredNumber.getImaginary(), 0);
    ...
    public void testPolar2Complex() {
        TestUtils.assertEquals(Complex.ONE, Complex.fromPolar(1, 0), 10e-12);
        TestUtils.assertEquals(Complex.ZERO, Complex.fromPolar(0, 1), 10e-12);
        TestUtils.assertEquals(Complex.ZERO, Complex.fromPolar(0, -1), 10e-12);
        TestUtils.assertEquals(Complex.I, Complex.fromPolar(1, pi / 2), 10e-12);
        TestUtils.assertEquals(Complex.I.negate(), Complex.fromPolar(1, -pi / 2), 10e-12);

        // test combinations of r[0,5) and various theta values (both positive and negative)
        double r = 0;
        for (int i = 0; i < 5; i++) {
            ...
            for (int j = 0; j < 20; j++) {
                theta += pi / 6;
                TestUtils.assertEquals(polarToComplexStub(r, theta), Complex.fromPolar(r, theta), 10e-12);
            }
    ...
    protected Complex polarToComplexStub(double r, double theta) {
    ...
    public void testPolar2ComplexNaN() {
        TestUtils.assertSame(Complex.NaN, Complex.fromPolar(nan, 1));
        TestUtils.assertSame(Complex.NaN, Complex.fromPolar(1, nan));
        TestUtils.assertSame(Complex.NaN, Complex.fromPolar(nan, nan));
    }

    @Test
    public void testPolar2ComplexInf() {
        TestUtils.assertSame(Complex.NaN, Complex.fromPolar(1, inf));
        TestUtils.assertSame(Complex.NaN, Complex.fromPolar(1, neginf));
        TestUtils.assertSame(Complex.NaN, Complex.fromPolar(inf, inf));
        TestUtils.assertSame(Complex.NaN, Complex.fromPolar(inf, neginf));
        TestUtils.assertSame(infInf, Complex.fromPolar(inf, pi / 4));
        TestUtils.assertSame(infNaN, Complex.fromPolar(inf, 0));
        TestUtils.assertSame(infNegInf, Complex.fromPolar(inf, -pi / 4));
        TestUtils.assertSame(negInfInf, Complex.fromPolar(inf, 3 * pi / 4));
        TestUtils.assertSame(negInfNegInf, Complex.fromPolar(inf, 5 * pi / 4));
    }
}